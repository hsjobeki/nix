...
struct NixRepl : AbstractNixRepl
#if HAVE_BOEHMGC
    ,
                 gc
#endif
{
  ...
  NixRepl(const SearchPath &searchPath, nix::ref<Store> store,
          ref<EvalState> state, std::function<AnnotatedValues()> getValues);
  virtual ~NixRepl();
  ...
  std::ostream &printValue(std::ostream &str, Value &v, unsigned int maxDepth,
                           ValuesSeen &seen);
  std::ostream &printValueAndDoc(std::ostream &str, Value &v,
...
NixRepl::NixRepl(const SearchPath &searchPath, nix::ref<Store> store,
                 ref<EvalState> state,
                 std::function<NixRepl::AnnotatedValues()> getValues)
    : AbstractNixRepl(state), debugTraceIndex(0), getValues(getValues),
...
static std::ostream &showDebugTrace(std::ostream &out,
...
void NixRepl::mainLoop() {
  ...
  while (true) {
    ...
    try {
      ...
      // when that session returns the exception will land here.  No need to
      // show it again; show the error for this repl session instead.
...
StringSet NixRepl::completePrefix(const std::string &prefix) {
  ...
  if ((slash = cur.rfind('/')) != std::string::npos) {
    ...
    try {
      ...
      state->forceAttrs(
          v, noPos,
          "while evaluating an attrset for the purpose of completion (this "
          "error should not be displayed; file an issue?)");
...
bool NixRepl::processLine(std::string line) {
  ...
  if (command == ":?" || command == ":help") {
    ...
    std::cout
        ...
        << "  :a, :add <expr>              Add attributes from resulting set "
           "to scope\n"
        << "  :b <expr>                    Build a derivation\n"
        << "  :bl <expr>                   Build a derivation, creating GC "
           "roots in the\n"
        << "                               working directory\n"
        << "  :e, :edit <expr>             Open package or function in "
           "$EDITOR\n"
        << "  :i <expr>                    Build derivation, then install "
           "result into\n"
        << "                               current profile\n"
        << "  :l, :load <path>             Load Nix expression and add it to "
           "scope\n"
        ...
        << "  :sh <expr>                   Build dependencies of derivation, "
           "then start\n"
        << "                               nix-shell\n"
        << "  :t <expr>                    Describe result of evaluation\n"
        << "  :u <expr>                    Build derivation, then start "
           "nix-shell\n"
        << "  :doc <expr>                  Show documentation of a builtin "
           "function\n"
        << "  :log <expr>                  Show logs for a derivation\n"
        << "  :te, :trace-enable [bool]    Enable, disable or toggle showing "
           "traces for\n"
        << "                               errors\n"
    ...
    if (state->debugRepl) {
      std::cout << "\n"
                ...
                << "  :c, :continue    Go until end of program, exception, or "
                   "builtins.break\n"
                << "  :s, :step        Go one step\n";
    }
    ...
    runProgram2(
        RunOptions{.program = editor, .searchPath = true, .args = args});
  ...
  else if (command == ":b" || command == ":bl" || command == ":i" ||
    ...
    if (command == ":b" || command == ":bl") {
      ...
      for (auto &sub : subs) {
        ...
        if (log) {
          printInfo("got build log for '%s' from '%s'", drvPathRaw,
                    logSub.getUri());
    ...
    if (auto doc = state->getDoc(v)) {
      ...
      // Try printing the RFC-145 doc comments
...
void NixRepl::loadFlake(const std::string &flakeRefS) {
  if (flakeRefS.empty())
    throw Error("cannot use ':load-flake' without a path specified. (Use '.' "
                "for the current working directory.)");

  auto flakeRef = parseFlakeRef(flakeRefS, absPath("."), true);
  if (evalSettings.pureEval && !flakeRef.input.isLocked())
    throw Error("cannot use ':load-flake' on locked flake reference '%s' (use "
                "--impure to override)",
                flakeRefS);
...
Expr *NixRepl::parseString(std::string s) {
  return state->parseExprFromString(
      std::move(s), state->rootPath(CanonPath::fromCwd()), staticEnv);
}
...
std::ostream &NixRepl::printValueAndDoc(std::ostream &str, Value &v,
                                        unsigned int maxDepth) {
  if (v.isLambda()) {
    auto pos = state->positions[v.lambda.fun->pos];
    tryPrintDoc(str, pos);
...
std::ostream &NixRepl::printValue(std::ostream &str, Value &v,
  ...
  case nAttrs: {
    ...
    if (isDrv) {
      ...
      NixStringContext context;
      if (i != v.attrs->end())
        str << state->store->printStorePath(state->coerceToStorePath(
            i->pos, *i->value, context,
...
AbstractNixRepl::create(const SearchPath &searchPath, nix::ref<Store> store,
                        ref<EvalState> state,
                        std::function<AnnotatedValues()> getValues) {
  return std::make_unique<NixRepl>(searchPath, openStore(), state, getValues);
}

void AbstractNixRepl::runSimple(ref<EvalState> evalState,
  ...
  auto repl =
      std::make_unique<NixRepl>(searchPath, openStore(), evalState, getValues);

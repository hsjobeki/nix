...
class BoehmGCStackAllocator : public StackAllocator {
  boost::coroutines2::protected_fixedsize_stack stack{
      ...
      std::max(boost::context::stack_traits::default_size(),
...
public:
  boost::context::stack_context allocate() override {
    ...
    GC_add_roots(static_cast<char *>(sctx.sp) - pfss_usable_stack_size(sctx),
                 sctx.sp);
    return sctx;
  }

  void deallocate(boost::context::stack_context sctx) override {
    GC_remove_roots(static_cast<char *>(sctx.sp) - pfss_usable_stack_size(sctx),
                    sctx.sp);
...
public:
  BoehmDisableGC() { GC_disable(); };
  ~BoehmDisableGC() { GC_enable(); };
};
#endif
...
ErrorBuilder &ErrorBuilder::withTrace(PosIdx pos, const std::string_view text) {
  info.traces.push_front(Trace{.pos = state.positions[pos],
                               .hint = hintformat(std::string(text)),
                               .frame = false});
  return *this;
}

ErrorBuilder &ErrorBuilder::withFrameTrace(PosIdx pos,
...
ErrorBuilder &ErrorBuilder::withFrame(const Env &env, const Expr &expr) {
  ...
  state.debugTraces.push_front(
      DebugTrace{.pos = nullptr,
                 .expr = expr,
                 .env = env,
...
EvalState::EvalState(const SearchPath &_searchPath, ref<Store> store,
                     std::shared_ptr<Store> buildStore)
    : sWith(symbols.create("<with>")), sOutPath(symbols.create("outPath")),
      sDrvPath(symbols.create("drvPath")), sType(symbols.create("type")),
      sMeta(symbols.create("meta")), sName(symbols.create("name")),
      sValue(symbols.create("value")), sSystem(symbols.create("system")),
      sOverrides(symbols.create("__overrides")),
      sOutputs(symbols.create("outputs")),
      sOutputName(symbols.create("outputName")),
      sIgnoreNulls(symbols.create("__ignoreNulls")),
      sFile(symbols.create("file")), sLine(symbols.create("line")),
      sColumn(symbols.create("column")), sFunctor(symbols.create("__functor")),
      sToString(symbols.create("__toString")), sRight(symbols.create("right")),
      sWrong(symbols.create("wrong")),
      sStructuredAttrs(symbols.create("__structuredAttrs")),
      sBuilder(symbols.create("builder")), sArgs(symbols.create("args")),
      ...
      sSelf(symbols.create("self")), sEpsilon(symbols.create("")),
      sStartSet(symbols.create("startSet")),
      sOperator(symbols.create("operator")), sKey(symbols.create("key")),
      sPath(symbols.create("path")), sPrefix(symbols.create("prefix")),
      sOutputSpecified(symbols.create("outputSpecified")), repair(NoRepair),
      emptyBindings(0),
      derivationInternal(rootPath(CanonPath("/builtin/derivation.nix"))),
      store(store), buildStore(buildStore ? buildStore : store),
      debugRepl(nullptr), debugStop(false), debugQuit(false), trylevel(0),
...
#endif
      ,
      baseEnv(allocEnv(128)), staticBaseEnv{
                                  std::make_shared<StaticEnv>(false, nullptr)} {
...
void EvalState::allowAndSetStorePathString(const StorePath &storePath,
                                           Value &v) {
  allowPath(storePath);
...
SourcePath EvalState::checkSourcePath(const SourcePath &path_) {
  ...
  if (hasPrefix(abspath, corepkgsPrefix))
    return CanonPath(abspath);
  ...
  if (!found) {
    auto modeInformation =
        evalSettings.pureEval ? "in pure eval mode (use '--impure' to override)"
                              : "in restricted mode";
    throw RestrictedPathError("access to absolute path '%1%' is forbidden %2%",
                              abspath, modeInformation);
  }
  ...
  throw RestrictedPathError(
      "access to canonical path '%1%' is forbidden in restricted mode", path);
}
...
void printEnvBindings(const SymbolTable &st, const StaticEnv &se,
                      const Env &env, int lvl) {
...
void mapStaticEnvBindings(const SymbolTable &st, const StaticEnv &se,
                          const Env &env, ValMap &vm) {
  // add bindings for the next level up first, so that the bindings for this
  // level override the higher levels. The top level bindings (builtins) are
  // skipped since they are added for us by initEnv()
  if (env.up && se.up) {
...
std::unique_ptr<ValMap> mapStaticEnvBindings(const SymbolTable &st,
                                             const StaticEnv &se,
                                             const Env &env) {
...
void EvalState::runDebugRepl(const Error *error, const Env &env,
                             const Expr &expr) {
          ...
          ? std::make_unique<DebugTraceStacker>(
                *this,
                DebugTrace{.pos =
                               error->info().errPos
                                   ? error->info().errPos
                                   : static_cast<std::shared_ptr<AbstractPos>>(
                                         positions[expr.getPos()]),
                           .expr = expr,
                           .env = env,
                           .hint = error->info().msg,
                           .isError = true})
  ...
  if (error) {
    ...
    printError(ANSI_BOLD "Starting REPL to allow you to inspect the current "
...
void EvalState::addErrorTrace(Error &e, const char *s,
                              const std::string &s2) const {
  e.addTrace(nullptr, s, s2);
}

void EvalState::addErrorTrace(Error &e, const PosIdx pos, const char *s,
                              const std::string &s2, bool frame) const {
  e.addTrace(positions[pos], hintfmt(s, s2), frame);
}

static std::unique_ptr<DebugTraceStacker>
makeDebugTraceStacker(EvalState &state, Expr &expr, Env &env,
                      std::shared_ptr<AbstractPos> &&pos, const char *s,
                      const std::string &s2) {
  return std::make_unique<DebugTraceStacker>(state,
                                             DebugTrace{.pos = std::move(pos),
                                                        .expr = expr,
                                                        .env = env,
                                                        .hint = hintfmt(s, s2),
                                                        .isError = false});
...
DebugTraceStacker::DebugTraceStacker(EvalState &evalState, DebugTrace t)
    : evalState(evalState), trace(std::move(t)) {
  evalState.debugTraces.push_front(trace);
  if (evalState.debugStop && evalState.debugRepl)
    evalState.runDebugRepl(nullptr, trace.env, trace.expr);
}

void Value::mkString(std::string_view s) { mkString(makeImmutableString(s)); }
...
inline Value *EvalState::lookupVar(Env *env, const ExprVar &var, bool noEval) {
  ...
  while (1) {
    ...
    if (!env->prevWith)
      error("undefined variable '%1%'", symbols[var.name])
          .atPos(var.pos)
          .withFrame(*env, var)
...
void EvalState::mkStorePathString(const StorePath &p, Value &v) {
  v.mkString(store->printStorePath(p),
             NixStringContext{
...
std::string
EvalState::mkOutputStringRaw(const SingleDerivedPath::Built &b,
  ...
  return optStaticOutputPath
             ...
             : DownstreamPlaceholder::fromSingleDerivedPathBuilt(b, xpSettings)
                   .render();
}

void EvalState::mkOutputString(Value &value, const SingleDerivedPath::Built &b,
                               std::optional<StorePath> optStaticOutputPath,
                               const ExperimentalFeatureSettings &xpSettings) {
  value.mkString(mkOutputStringRaw(b, optStaticOutputPath, xpSettings),
                 NixStringContext{b});
}

std::string
EvalState::mkSingleDerivedPathStringRaw(const SingleDerivedPath &p) {
  return std::visit(
      overloaded{
                    ...
                    [&](const SingleDerivedPath::Opaque &o) {
                      ...
                      if (i == drv.outputs.end())
                        throw Error("derivation '%s' does not have output '%s'",
                                    b.drvPath->to_string(*store), b.output);
                      return i->second.path(*store, drv.name, b.output);
                    },
                    [&](const SingleDerivedPath::Built &o)
                        -> std::optional<StorePath> { return std::nullopt; },
                },
                b.drvPath->raw());
...
void EvalState::mkSingleDerivedPathString(const SingleDerivedPath &p,
                                          Value &v) {
  v.mkString(
...
void EvalState::cacheFile(const SourcePath &path,
                          const SourcePath &resolvedPath, Expr *e, Value &v,
                          bool mustBeTrivial) {
    ...
    auto dts =
        debugRepl
            ? makeDebugTraceStacker(
                  *this, *e, this->baseEnv,
                  e->getPos() ? static_cast<std::shared_ptr<AbstractPos>>(
                                    positions[e->getPos()])
                              : nullptr,
...
void ExprAttrs::eval(EvalState &state, Env &env, Value &v) {
  ...
  if (recursive) {
    ...
    if (hasOverrides) {
      ...
      state.forceAttrs(
          *vOverrides, [&]() { return vOverrides->determinePos(noPos); },
  ...
  for (auto &i : dynamicAttrs) {
    ...
    if (nameVal.type() == nNull)
      continue;
    state.forceStringNoCtx(nameVal, i.pos,
                           "while evaluating the name of a dynamic attribute");
    auto nameSym = state.symbols.create(nameVal.string.s);
    Bindings::iterator j = v.attrs->find(nameSym);
    if (j != v.attrs->end())
      state
          .error("dynamic attribute '%1%' already defined at %2%",
...
void ExprSelect::eval(EvalState &state, Env &env, Value &v) {
  ...
  try {
    auto dts =
        state.debugRepl
            ? makeDebugTraceStacker(state, *this, env, state.positions[pos2],
...
void EvalState::callFunction(Value &fun, size_t nrArgs, Value **args,
    ...
    if (vCur.isLambda()) {
          ...
          if (!j) {
            if (!i.def) {
              error("function '%1%' called without required argument '%2%'",
                    (lambda.name ? std::string(symbols[lambda.name])
                                 : "anonymous lambda"),
                    symbols[i.name])
                  .atPos(lambda.pos)
      ...
      if (nrArgs < argsLeft) {
        ...
        try {
          ...
          // 2. Create a fake env (arg1, arg2, etc.) and a fake expr (arg1:
          // arg2: etc: builtins.name arg1 arg2 etc)
          //    so the debugger allows to inspect the wrong parameters passed to
          //    the builtin.
      ...
      *args2[0] = vCur;
      try {
        ...
        e.addTrace(positions[pos], "while calling a functor (an attribute set "
                                   "with a '__functor' attribute)");
        throw;
      }
      nrArgs--;
      args++;
      ...
      error("attempt to call something which is not a function but %1%",
            showType(vCur))
          .atPos(pos)
          .debugThrow<TypeError>();
  }
...
void EvalState::autoCallFunction(Bindings &args, Value &fun, Value &res) {
  ...
  auto attrs = buildBindings(
      std::max(static_cast<uint32_t>(fun.lambda.fun->formals->formals.size()),
               args.size()));
...
void ExprIf::eval(EvalState &state, Env &env, Value &v) {
  // We cheat in the parser, and pass the position of the condition as the
  // position of the if itself.
  (state.evalBool(env, cond, pos, "while evaluating a branch condition")
       ? then
       : else_)
      ->eval(state, env, v);
}
...
void ExprAssert::eval(EvalState &state, Env &env, Value &v) {
  if (!state.evalBool(env, cond, pos,
    ...
    state.error("assertion '%1%' failed", out.str())
        .atPos(pos)
        .withFrame(env, *this)
...
void ExprOpNot::eval(EvalState &state, Env &env, Value &v) {
  v.mkBool(!state.evalBool(
      env, e, noPos, "in the argument of the not operator")); // XXX: FIXME: !
}
...
void ExprOpEq::eval(EvalState &state, Env &env, Value &v) {
  ...
  v.mkBool(
      state.eqValues(v1, v2, pos, "while testing two values for equality"));
}
...
void ExprOpNEq::eval(EvalState &state, Env &env, Value &v) {
  ...
  e2->eval(state, env, v2);
  v.mkBool(
      !state.eqValues(v1, v2, pos, "while testing two values for inequality"));
}

void ExprOpAnd::eval(EvalState &state, Env &env, Value &v) {
  v.mkBool(state.evalBool(env, e1, pos,
                          "in the left operand of the AND (&&) operator") &&
           state.evalBool(env, e2, pos,
...
void ExprOpUpdate::eval(EvalState &state, Env &env, Value &v) {
  Value v1, v2;
  state.evalAttrs(env, e1, v1, pos,
                  "in the left operand of the update (//) operator");
  state.evalAttrs(env, e2, v2, pos,
...
void ExprConcatStrings::eval(EvalState &state, Env &env, Value &v) {
  ...
  for (auto &[i_pos, i] : *es) {
    ...
    if (firstType == nInt) {
      if (vTmp.type() == nInt) {
        ...
        state.error("cannot add %1% to an integer", showType(vTmp))
            .atPos(i_pos)
            .withFrame(env, *this)
            .debugThrow<EvalError>();
      ...
      if (vTmp.type() == nInt) {
        ...
        state.error("cannot add %1% to a float", showType(vTmp))
            .atPos(i_pos)
            .withFrame(env, *this)
            .debugThrow<EvalError>();
    } else {
      ...
      auto part = state.coerceToString(i_pos, vTmp, context,
                                       "while evaluating a path segment", false,
                                       firstType == nString, !first);
      sSize += part->size();
  ...
  if (firstType == nInt)
    ...
    if (!context.empty())
      state
          .error("a string that refers to a store path cannot be appended to a "
                 "path")
          .atPos(pos)
          .withFrame(env, *this)
          .debugThrow<EvalError>();
    ...
    if (v.type() == nAttrs) {
      for (auto &i : *v.attrs)
        try {
          ...
          // necessary.
          auto dts =
              debugRepl && i.value->isThunk()
                  ? makeDebugTraceStacker(
                        *this, *i.value->thunk.expr, *i.value->thunk.env,
                        positions[i.pos],
          ...
          addErrorTrace(e, i.pos, "while evaluating the attribute '%1%'",
                        symbols[i.name]);
          throw;
        }
    }
...
NixInt EvalState::forceInt(Value &v, const PosIdx pos,
  ...
  try {
    forceValue(v, pos);
    if (v.type() != nInt)
      error("value is %1% while an integer was expected", showType(v))
          .debugThrow<TypeError>();
    return v.integer;
  } catch (Error &e) {
    e.addTrace(positions[pos], errorCtx);
...
NixFloat EvalState::forceFloat(Value &v, const PosIdx pos,
  ...
  try {
    ...
    if (v.type() == nInt)
      ...
      error("value is %1% while a float was expected", showType(v))
          .debugThrow<TypeError>();
    return v.fpoint;
  } catch (Error &e) {
    e.addTrace(positions[pos], errorCtx);
...
void EvalState::forceFunction(Value &v, const PosIdx pos,
  ...
  try {
    ...
    if (v.type() != nFunction && !isFunctor(v))
      error("value is %1% while a function was expected", showType(v))
          .debugThrow<TypeError>();
  } catch (Error &e) {
    e.addTrace(positions[pos], errorCtx);
    throw;
...
std::string_view EvalState::forceString(Value &v, const PosIdx pos,
  ...
  try {
    forceValue(v, pos);
    if (v.type() != nString)
      error("value is %1% while a string was expected", showType(v))
          .debugThrow<TypeError>();
    return v.string.s;
  } catch (Error &e) {
    e.addTrace(positions[pos], errorCtx);
...
std::string_view EvalState::forceString(Value &v, NixStringContext &context,
                                        const PosIdx pos,
                                        std::string_view errorCtx) {
...
std::string_view EvalState::forceStringNoCtx(Value &v, const PosIdx pos,
  ...
  auto s = forceString(v, pos, errorCtx);
  if (v.string.context) {
    error("the string '%1%' is not allowed to refer to a store path (such as "
          "'%2%')",
...
EvalState::tryAttrsToString(const PosIdx pos, Value &v,
...
BackedStringView EvalState::coerceToString(const PosIdx pos, Value &v,
                                           NixStringContext &context,
                                           std::string_view errorCtx,
  ...
  if (v.type() == nAttrs) {
    ...
    return coerceToString(pos, *i->value, context, errorCtx, coerceMore,
                          copyToStore, canonicalizePath);
  }

  if (v.type() == nExternal) {
    try {
      return v.external->coerceToString(positions[pos], context, coerceMore,
                                        copyToStore);
    } catch (Error &e) {
...
StorePath EvalState::copyPathToStore(NixStringContext &context,
                                     const SourcePath &path) {
  if (nix::isDerivation(path.path.abs()))
    error("file names are not allowed to end in '%1%'", drvExtension)
        .debugThrow<EvalError>();

  auto i = srcToStore.find(path);

  auto dstPath = i != srcToStore.end() ? i->second : [&]() {
    auto dstPath = path.fetchToStore(store, path.baseName(), nullptr, repair);
    allowPath(dstPath);
    srcToStore.insert_or_assign(path, dstPath);
    printMsg(lvlChatty, "copied source '%1%' -> '%2%'", path,
...
SourcePath EvalState::coerceToPath(const PosIdx pos, Value &v,
                                   NixStringContext &context,
                                   std::string_view errorCtx) {
  auto path =
      coerceToString(pos, v, context, errorCtx, false, false, true).toOwned();
  if (path == "" || path[0] != '/')
    error("string '%1%' doesn't represent an absolute path", path)
        .withTrace(pos, errorCtx)
        .debugThrow<EvalError>();
  return CanonPath(path);
}

StorePath EvalState::coerceToStorePath(const PosIdx pos, Value &v,
                                       NixStringContext &context,
                                       std::string_view errorCtx) {
  auto path =
      coerceToString(pos, v, context, errorCtx, false, false, true).toOwned();
  if (auto storePath = store->maybeParseStorePath(path))
    return *storePath;
  error("path '%1%' is not in the Nix store", path)
      .withTrace(pos, errorCtx)
      .debugThrow<EvalError>();
}

std::pair<SingleDerivedPath, std::string_view>
EvalState::coerceToSingleDerivedPathUnchecked(const PosIdx pos, Value &v,
                                              std::string_view errorCtx) {
  NixStringContext context;
  auto s = forceString(v, context, pos, errorCtx);
  auto csize = context.size();
  if (csize != 1)
    error("string '%s' has %d entries in its context. It should only have "
          "exactly one entry",
          s, csize)
        .withTrace(pos, errorCtx)
        .debugThrow<EvalError>();
  auto derivedPath = std::visit(
      overloaded{
          ...
          [&](NixStringContextElem::DrvDeep &&) -> SingleDerivedPath {
            error("string '%s' has a context which refers to a complete source "
                  "and binary closure. This is not supported at this time",
                  s)
                .withTrace(pos, errorCtx)
                .debugThrow<EvalError>();
          },
...
SingleDerivedPath
EvalState::coerceToSingleDerivedPath(const PosIdx pos, Value &v,
                                     std::string_view errorCtx) {
  ...
  if (s != sExpected) {
    ...
    std::visit(
        overloaded{
            [&](const SingleDerivedPath::Opaque &o) {
              error("path string '%s' has context with the different path '%s'",
                    s, sExpected)
                  .withTrace(pos, errorCtx)
                  .debugThrow<EvalError>();
            },
            [&](const SingleDerivedPath::Built &b) {
              error("string '%s' has context with the output '%s' from "
...
bool EvalState::eqValues(Value &v1, Value &v2, const PosIdx pos,
                         std::string_view errorCtx) {
  forceValue(v1, noPos);
  forceValue(v2, noPos);
  ...
  default:
    error("cannot compare %1% with %2%", showType(v1), showType(v2))
        .withTrace(pos, errorCtx)
        .debugThrow<EvalError>();
  }
}
...
std::string ExternalValueBase::coerceToString(const Pos &pos,
                                              NixStringContext &context,
                                              bool copyMore,

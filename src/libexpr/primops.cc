...
static SourcePath realisePath(EvalState &state, const PosIdx pos, Value &v,
  ...
  try {
    ...
    auto realPath = state.rootPath(CanonPath(
        state.toRealPath(rewriteStrings(path.path.abs(), rewrites), context)));

    return flags.checkForPureEval ? state.checkSourcePath(realPath) : realPath;
  } catch (Error &e) {
    e.addTrace(state.positions[pos], "while realising the context of path '%s'",
               path);
    throw;
  }
}
 ...
 */
static void mkOutputString(EvalState &state, BindingsBuilder &attrs,
...
/* Load and evaluate an expression from path specified by the
   argument. */
static void import(EvalState &state, const PosIdx pos, Value &vPath,
                   Value *vScope, Value &v) {
...
#include "imported-drv-to-derivation.nix.gen.hh"
    ...
    state.forceFunction(
        **state.vImportedDrvToDerivation, pos,
        "while evaluating imported-drv-to-derivation.nix.gen.hh");
    v.mkApp(*state.vImportedDrvToDerivation, w);
    state.forceAttrs(v, pos,
                     "while calling imported-drv-to-derivation.nix.gen.hh");
  }
...
#include "fetchurl.nix.gen.hh"
    ...
    if (!vScope)
      ...
      auto staticEnv = std::make_shared<StaticEnv>(
          false, state.staticBaseEnv.get(), vScope->attrs->size());
...
void prim_importNative(EvalState &state, const PosIdx pos, Value **args,
                       Value &v) {
  auto path = realisePath(state, pos, *args[0]);

  std::string sym(state.forceStringNoCtx(
      *args[1], pos,
      "while evaluating the second argument passed to builtins.importNative"));

  void *handle = dlopen(path.path.c_str(), RTLD_LAZY | RTLD_LOCAL);
  if (!handle)
    state.debugThrowLastTrace(
        EvalError("could not open '%1%': %2%", path, dlerror()));
  ...
  if (!func) {
    ...
    if (message)
      state.debugThrowLastTrace(EvalError(
          "could not load symbol '%1%' from '%2%': %3%", sym, path, message));
    else
      state.debugThrowLastTrace(
          EvalError("symbol '%1%' from '%2%' resolved to NULL when a function "
...
void prim_exec(EvalState &state, const PosIdx pos, Value **args, Value &v) {
  state.forceList(
      *args[0], pos,
      "while evaluating the first argument passed to builtins.exec");
  auto elems = args[0]->listElems();
  auto count = args[0]->listSize();
  if (count == 0)
    state.error("at least one argument to 'exec' required")
        .atPos(pos)
        .debugThrow<EvalError>();
  NixStringContext context;
  auto program = state
                     .coerceToString(pos, *elems[0], context,
                                     "while evaluating the first element of "
                                     "the argument passed to builtins.exec",
                                     false, false)
                     .toOwned();
  Strings commandArgs;
    ...
    commandArgs.push_back(
        state
            .coerceToString(pos, *elems[i], context,
                            "while evaluating an element of the argument "
                            "passed to builtins.exec",
                            false, false)
            .toOwned());
  }
  try {
    ...
    state
        .error("cannot execute '%1%', since path '%2%' is not valid", program,
               e.path)
        .atPos(pos)
        .debugThrow<EvalError>();
  }
  ...
  try {
    ...
    e.addTrace(state.positions[pos], "while parsing the output from '%1%'",
               program);
    throw;
  }
  try {
    ...
    e.addTrace(state.positions[pos], "while evaluating the output from '%1%'",
               program);
    throw;
  }
}
...
static void prim_typeOf(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
...
static void prim_isNull(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  state.forceValue(*args[0], pos);
...
static void prim_isFunction(EvalState &state, const PosIdx pos, Value **args,
                            Value &v) {
  state.forceValue(*args[0], pos);
...
static void prim_isInt(EvalState &state, const PosIdx pos, Value **args,
                       Value &v) {
  state.forceValue(*args[0], pos);
...
static void prim_isFloat(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
  state.forceValue(*args[0], pos);
...
static void prim_isString(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
  state.forceValue(*args[0], pos);
...
static void prim_isBool(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  state.forceValue(*args[0], pos);
...
static void prim_isPath(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  state.forceValue(*args[0], pos);
...
struct CompareValues {
  ...
  CompareValues(EvalState &state, const PosIdx pos,
                const std::string_view &&errorCtx)
      : state(state), pos(pos), errorCtx(errorCtx){};
  ...
  bool operator()(Value *v1, Value *v2, std::string_view errorCtx) const {
    try {
      ...
      if (v1->type() != v2->type())
        state.error("cannot compare %s with %s", showType(*v1), showType(*v2))
            .debugThrow<EvalError>();
...
#pragma GCC diagnostic ignored "-Wswitch-enum"
      ...
      default:
        state
            .error("cannot compare %s with %s; values of that type are "
                   "incomparable",
                   showType(*v1), showType(*v2))
            .debugThrow<EvalError>();
...
static Bindings::iterator getAttr(EvalState &state, Symbol attrSym,
                                  Bindings *attrSet,
                                  std::string_view errorCtx) {
  Bindings::iterator value = attrSet->find(attrSym);
  if (value == attrSet->end()) {
    state.error("attribute '%s' missing", state.symbols[attrSym])
...
static void prim_genericClosure(EvalState &state, const PosIdx pos,
                                Value **args, Value &v) {
  state.forceAttrs(
      *args[0], noPos,
  ...
  Bindings::iterator startSet =
      getAttr(state, state.sStartSet, args[0]->attrs,
              "in the attrset passed as argument to builtins.genericClosure");

  state.forceList(*startSet->value, noPos,
                  "while evaluating the 'startSet' attribute passed as "
                  "argument to builtins.genericClosure");
  ...
  Bindings::iterator op =
      getAttr(state, state.sOperator, args[0]->attrs,
              "in the attrset passed as argument to builtins.genericClosure");
  state.forceFunction(*op->value, noPos,
                      "while evaluating the 'operator' attribute passed as "
                      "argument to builtins.genericClosure");
  ...
  while (!workSet.empty()) {
    ...
    state.forceAttrs(*e, noPos,
                     "while evaluating one of the elements generated by (or "
                     "initially passed to) builtins.genericClosure");

    Bindings::iterator key =
        getAttr(state, state.sKey, e->attrs,
                "in one of the attrsets generated by (or initially passed to) "
    ...
    state.forceList(newElements, noPos,
                    "while evaluating the return value of the `operator` "
    ...
    for (auto elem : newElements.listItems()) {
      state.forceValue(
          *elem,
          noPos); // "while evaluating one one of the elements returned by the
                  // `operator` passed to builtins.genericClosure");
      workSet.push_back(elem);
    }
  }
     ...
     .fun = [](EvalState &state, const PosIdx pos, Value **args, Value &v) {
       ...
       auto s = state
                    .coerceToString(pos, *args[0], context,
                                    "while evaluating the error message passed "
                                    "to builtins.abort")
                    .toOwned();
       state.debugThrowLastTrace(Abort(
           "evaluation aborted with the following error message: '%1%'", s));
     }});
...
static void prim_addErrorContext(EvalState &state, const PosIdx pos,
                                 Value **args, Value &v) {
  try {
    ...
    auto message = state
                       .coerceToString(pos, *args[0], context,
                                       "while evaluating the error message "
                                       "passed to builtins.addErrorContext",
                                       false, false)
                       .toOwned();
...
static void prim_ceil(EvalState &state, const PosIdx pos, Value **args,
                      Value &v) {
  auto value = state.forceFloat(
      *args[0], args[0]->determinePos(pos),
...
static void prim_floor(EvalState &state, const PosIdx pos, Value **args,
                       Value &v) {
  auto value = state.forceFloat(
      *args[0], args[0]->determinePos(pos),
...
static void prim_tryEval(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
...
static void prim_getEnv(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  std::string name(state.forceStringNoCtx(
      *args[0], pos,
      "while evaluating the first argument passed to builtins.getEnv"));
  v.mkString(evalSettings.restrictEval || evalSettings.pureEval
                 ? ""
...
static void prim_seq(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
  state.forceValue(*args[0], pos);
  state.forceValue(*args[1], pos);
  v = *args[1];
...
static void prim_deepSeq(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
...
static void prim_trace(EvalState &state, const PosIdx pos, Value **args,
                       Value &v) {
  state.forceValue(*args[0], pos);
 ...
 */
static void prim_second(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  state.forceValue(*args[1], pos);
  v = *args[1];
}
...
static void derivationStrictInternal(EvalState &state, const std::string &name,
                                     Bindings *attrs, Value &v);
...
/* Construct (as a unobservable side effect) a Nix derivation
   ...
   derivation. */
static void prim_derivationStrict(EvalState &state, const PosIdx pos,
                                  Value **args, Value &v) {
  state.forceAttrs(
      *args[0], pos,
  ...
  Bindings::iterator nameAttr =
      getAttr(state, state.sName, attrs,
              "in the attrset passed as argument to builtins.derivationStrict");

  std::string drvName;
  try {
    drvName = state.forceStringNoCtx(*nameAttr->value, pos,
                                     "while evaluating the `name` attribute "
                                     "passed to builtins.derivationStrict");
  } catch (Error &e) {
    e.addTrace(state.positions[nameAttr->pos],
...
static void derivationStrictInternal(EvalState &state,
  ...
  for (auto &i : attrs->lexicographicOrder(state.symbols)) {
    ...
    auto handleHashMode = [&](const std::string_view s) {
      if (s == "recursive")
        ...
        state.debugThrowLastTrace(EvalError(
            {.msg = hintfmt("invalid value '%s' for 'outputHashMode' attribute",
                            s),
    ...
    auto handleOutputs = [&](const Strings &ss) {
      ...
      for (auto &j : ss) {
        if (outputs.find(j) != outputs.end())
          state.debugThrowLastTrace(
              EvalError({.msg = hintfmt("duplicate derivation output '%1%'", j),
                         .errPos = state.positions[noPos]}));
        ...
        if (j == "drv")
          state.debugThrowLastTrace(
              EvalError({.msg = hintfmt("invalid derivation output name 'drv'"),
                         .errPos = state.positions[noPos]}));
        outputs.insert(j);
      }
      if (outputs.empty())
    ...
    try {
      ...
      /* All other attributes are passed to the builder through
        ...
        if (jsonObject) {
          ...
          if (i->name == state.sBuilder)
            drv.builder =
                state.forceString(*i->value, context, noPos, context_below);
          else if (i->name == state.sSystem)
            drv.platform =
                state.forceStringNoCtx(*i->value, noPos, context_below);
            ...
            for (auto elem : i->value->listItems())
              ss.emplace_back(
                  state.forceStringNoCtx(*elem, noPos, context_below));
            handleOutputs(ss);
          }

        } else {
          auto s = state
                       .coerceToString(noPos, *i->value, context, context_below,
                                       true)
                       .toOwned();
      ...
      e.addTrace(state.positions[i->pos],
                 hintfmt("while evaluating attribute '%1%' of derivation '%2%'",
                         key, drvName),
                 true);
      throw;
    }
  ...
  if (drv.builder == "")
    state.debugThrowLastTrace(
        EvalError({.msg = hintfmt("required attribute 'builder' missing"),
                   .errPos = state.positions[noPos]}));

  if (drv.platform == "")
    state.debugThrowLastTrace(
        EvalError({.msg = hintfmt("required attribute 'system' missing"),
                   .errPos = state.positions[noPos]}));
  ...
  if (isDerivation(drvName) &&
    ...
    state.debugThrowLastTrace(EvalError(
        {.msg = hintfmt("derivation names are allowed to end in '%s' only if "
                        "they produce a single derivation file",
                        drvExtension),
         .errPos = state.positions[noPos]}));
  }
  ...
  if (outputHash) {
    ...
    if (outputs.size() != 1 || *(outputs.begin()) != "out")
      state.debugThrowLastTrace(
          Error({.msg = hintfmt("multiple outputs are not supported in "
    ...
    drv.env["out"] =
        state.store->printStorePath(dof.path(*state.store, drvName, "out"));
...
/* Return a placeholder string for the specified output that will be
   ...
   ‘out’. */
static void prim_placeholder(EvalState &state, const PosIdx pos, Value **args,
                             Value &v) {
...
static void prim_toPath(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  NixStringContext context;
...
/* Allow a valid store path to be used in an expression.  This is
   ...
   corner cases. */
static void prim_storePath(EvalState &state, const PosIdx pos, Value **args,
                           Value &v) {
  if (evalSettings.pureEval)
    state.debugThrowLastTrace(
        EvalError({.msg = hintfmt("'%s' is not allowed in pure evaluation mode",
                                  "builtins.storePath"),
                   .errPos = state.positions[pos]}));
  ...
  auto path = state
                  .checkSourcePath(
                      state.coerceToPath(pos, *args[0], context,
                                         "while evaluating the first argument "
                                         "passed to builtins.storePath"))
                  .path;
  ...
  if (!state.store->isInStore(path.abs()))
    state.debugThrowLastTrace(
        EvalError({.msg = hintfmt("path '%1%' is not in the Nix store", path),
                   .errPos = state.positions[pos]}));
...
static void prim_baseNameOf(EvalState &state, const PosIdx pos, Value **args,
                            Value &v) {
  NixStringContext context;
  v.mkString(
...
/* Return the directory of the given path, i.e., everything before the
   ...
   of the argument. */
static void prim_dirOf(EvalState &state, const PosIdx pos, Value **args,
                       Value &v) {
  state.forceValue(*args[0], pos);
  if (args[0]->type() == nPath) {
    ...
    auto path = state.coerceToString(
        pos, *args[0], context,
        "while evaluating the first argument passed to 'builtins.dirOf'", false,
        false);
    auto dir = dirOf(*path);
...
static void prim_readFile(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
  auto path = realisePath(state, pos, *args[0]);
  ...
  if (state.store->isInStore(path.path.abs())) {
    try {
      refs =
          state.store
              ->queryPathInfo(state.store->toStorePath(path.path.abs()).first)
              ->references;
    } catch (Error &) { // FIXME: should be InvalidPathError
    }
    // Re-scan references to filter down to just the ones that actually occur in
    // the file.
...
static void prim_findFile(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
  state.forceList(
      *args[0], pos,
  ...
  for (auto v2 : args[0]->listItems()) {
    ...
    if (i != v2->attrs->end())
      prefix = state.forceStringNoCtx(
          *i->value, pos,
          "while evaluating the `prefix` attribute of an element of the list "
          "passed to builtins.findFile");

    i = getAttr(state, state.sPath, v2->attrs,
                "in an element of the __nixPath");

    NixStringContext context;
    auto path =
        state
            .coerceToString(pos, *i->value, context,
                            "while evaluating the `path` attribute of an "
                            "element of the list passed to builtins.findFile",
                            false, false)
            .toOwned();
    ...
    try {
      ...
      state.debugThrowLastTrace(EvalError(
          {.msg = hintfmt("cannot find '%1%', since path '%2%' is not valid",
                          path, e.path),
           .errPos = state.positions[pos]}));
    }
...
static void prim_hashFile(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
  ...
  if (!ht)
    state.debugThrowLastTrace(
        Error({.msg = hintfmt("unknown hash type '%1%'", type),
               .errPos = state.positions[pos]}));
...
static void prim_readFileType(EvalState &state, const PosIdx pos, Value **args,
                              Value &v) {
  auto path = realisePath(state, pos, *args[0]);
...
static void prim_readDir(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
  auto path = realisePath(state, pos, *args[0]);
...
static void prim_toXML(EvalState &state, const PosIdx pos, Value **args,
                       Value &v) {
...
static void prim_toJSON(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  std::ostringstream out;
  NixStringContext context;
...
static void prim_fromJSON(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
...
static void prim_toFile(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  NixStringContext context;
  std::string name(state.forceStringNoCtx(
      *args[0], pos,
...
static void addPath(EvalState &state, const PosIdx pos, std::string_view name,
                    Path path, Value *filterFun, FileIngestionMethod method,
                    const std::optional<Hash> expectedHash, Value &v,
  ...
  try {
    ...
    PathFilter filter = filterFun ? ([&](const Path &path) {
      ...
      Value arg2;
      arg2.mkString(S_ISREG(st.st_mode)   ? "regular"
    ...
    if (expectedHash)
      expectedStorePath =
          state.store->makeFixedOutputPath(name, FixedOutputInfo{
                                                     .method = method,
                                                     .hash = *expectedHash,
                                                     .references = {},
                                                 });
    ...
    if (!expectedHash || !state.store->isValidPath(*expectedStorePath)) {
      StorePath dstPath =
          settings.readOnlyMode
              ? state.store
                    ->computeStorePathForPath(name, path, method, htSHA256,
                                              filter)
                    .first
              : state.store->addToStore(name, path, method, htSHA256, filter,
                                        state.repair, refs);
...
static void prim_filterSource(EvalState &state, const PosIdx pos, Value **args,
                              Value &v) {
  NixStringContext context;
  auto path =
      state.coerceToPath(pos, *args[1], context,
                         "while evaluating the second argument (the path to "
                         "filter) passed to builtins.filterSource");
  state.forceFunction(
      *args[0], pos,
      "while evaluating the first argument passed to builtins.filterSource");
  addPath(state, pos, path.baseName(), path.path.abs(), args[0],
          FileIngestionMethod::Recursive, std::nullopt, v, context);
}
...
static void prim_path(EvalState &state, const PosIdx pos, Value **args,
                      Value &v) {
    ...
    if (n == "path")
      ...
      method = FileIngestionMethod{
          state.forceBool(*attr.value, attr.pos,
                          "while evaluating the `recursive` attribute passed "
                          "to builtins.path")};
    else if (n == "sha256")
      expectedHash = newHashAllowEmpty(
          state.forceStringNoCtx(*attr.value, attr.pos,
                                 "while evaluating the `sha256` attribute "
                                 "passed to builtins.path"),
          htSHA256);
    else
      state.debugThrowLastTrace(
          EvalError({.msg = hintfmt("unsupported argument '%1%' to 'addPath'",
                                    state.symbols[attr.name]),
                     .errPos = state.positions[attr.pos]}));
  }
  if (!path)
    state.debugThrowLastTrace(
        EvalError({.msg = hintfmt("missing required 'path' attribute in the "
                                  "first argument to builtins.path"),
                   .errPos = state.positions[pos]}));
  if (name.empty())
    name = path->baseName();

  addPath(state, pos, name, path->path.abs(), filterFun, method, expectedHash,
          v, context);
}
...
/* Return the names of the attributes in a set as a sorted list of
   strings. */
static void prim_attrNames(EvalState &state, const PosIdx pos, Value **args,
                           Value &v) {
  state.forceAttrs(
      *args[0], pos,
...
/* Return the values of the attributes in a set as a list, in the same
   order as attrNames. */
static void prim_attrValues(EvalState &state, const PosIdx pos, Value **args,
                            Value &v) {
  state.forceAttrs(
      *args[0], pos,
...
void prim_getAttr(EvalState &state, const PosIdx pos, Value **args, Value &v) {
  ...
  Bindings::iterator i =
      getAttr(state, state.symbols.create(attr), args[1]->attrs,
...
static void prim_unsafeGetAttrPos(EvalState &state, const PosIdx pos,
                                  Value **args, Value &v) {
...
/* Dynamic version of the `?' operator. */
static void prim_hasAttr(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
  auto attr = state.forceStringNoCtx(
      *args[0], pos,
  ...
  v.mkBool(args[1]->attrs->find(state.symbols.create(attr)) !=
           args[1]->attrs->end());
}
...
static void prim_isAttrs(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
  state.forceValue(*args[0], pos);
...
static void prim_removeAttrs(EvalState &state, const PosIdx pos, Value **args,
                             Value &v) {
  state.forceAttrs(
      *args[0], pos,
  ...
  for (auto elem : args[1]->listItems()) {
    state.forceStringNoCtx(*elem, pos,
                           "while evaluating the values of the second argument "
                           "passed to builtins.removeAttrs");
  ...
  std::set_difference(args[0]->attrs->begin(), args[0]->attrs->end(),
                      names.begin(), names.end(), std::back_inserter(attrs));
...
static void prim_listToAttrs(EvalState &state, const PosIdx pos, Value **args,
                             Value &v) {
  state.forceList(
      *args[0], pos,
  ...
  for (auto v2 : args[0]->listItems()) {
    state.forceAttrs(*v2, pos,
                     "while evaluating an element of the list passed to "
                     "builtins.listToAttrs");

    Bindings::iterator j = getAttr(state, state.sName, v2->attrs,
                                   "in a {name=...; value=...;} pair");

    auto name = state.forceStringNoCtx(
        *j->value, j->pos,
        "while evaluating the `name` attribute of an element of the list "
        "passed to builtins.listToAttrs");

    auto sym = state.symbols.create(name);
    if (seen.insert(sym).second) {
      Bindings::iterator j2 = getAttr(state, state.sValue, v2->attrs,
                                      "in a {name=...; value=...;} pair");
...
static void prim_intersectAttrs(EvalState &state, const PosIdx pos,
                                Value **args, Value &v) {
  state.forceAttrs(
      *args[0], pos,
  ...
  // linear in the largest set, which is not acceptable for callPackage in
  // Nixpkgs.
  //
  ...
  // See "Fast Intersection Algorithms for Sorted Sequences" by Baeza-Yates et
  // al. https://cs.uwaterloo.ca/~ajsaling/papers/intersection_alg_app10.pdf The
  // only downsides I can think of are not having a linear access pattern for
  // similar sets, and having to maintain a more intricate algorithm.
  //
  // Adaptive
  // ===
...
static void prim_catAttrs(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
  ...
  for (auto v2 : args[1]->listItems()) {
    state.forceAttrs(*v2, pos,
                     "while evaluating an element in the list passed as second "
                     "argument to builtins.catAttrs");
...
static void prim_functionArgs(EvalState &state, const PosIdx pos, Value **args,
                              Value &v) {
  state.forceValue(*args[0], pos);
  ...
  if (!args[0]->isLambda())
    state.debugThrowLastTrace(
        TypeError({.msg = hintfmt("'functionArgs' requires a function"),
                   .errPos = state.positions[pos]}));
  ...
  auto attrs =
      state.buildBindings(args[0]->lambda.fun->formals->formals.size());
...
/*  */
static void prim_mapAttrs(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
  state.forceAttrs(
      *args[1], pos,
...
static void prim_zipAttrsWith(EvalState &state, const PosIdx pos, Value **args,
                              Value &v) {
  ...
  for (unsigned int n = 0; n < listSize; ++n) {
    ...
    state.forceAttrs(*vElem, noPos,
                     "while evaluating a value of the list passed as second "
                     "argument to builtins.zipAttrsWith");
...
static void prim_isList(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  state.forceValue(*args[0], pos);
...
static void elemAt(EvalState &state, const PosIdx pos, Value &list, int n,
                   Value &v) {
  state.forceList(
      list, pos,
  ...
  if (n < 0 || (unsigned int)n >= list.listSize())
    state.debugThrowLastTrace(
        Error({.msg = hintfmt("list index %1% is out of bounds", n),
               .errPos = state.positions[pos]}));
...
static void prim_elemAt(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  elemAt(state, pos, *args[0],
         state.forceInt(
             *args[1], pos,
...
static void prim_head(EvalState &state, const PosIdx pos, Value **args,
                      Value &v) {
...
static void prim_tail(EvalState &state, const PosIdx pos, Value **args,
                      Value &v) {
  state.forceList(
      *args[0], pos,
  ...
  if (args[0]->listSize() == 0)
    state.debugThrowLastTrace(
        Error({.msg = hintfmt("'tail' called on an empty list"),
               .errPos = state.positions[pos]}));
...
static void prim_map(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
  state.forceList(
      *args[1], pos,
  ...
  for (unsigned int n = 0; n < v.listSize(); ++n)
    (v.listElems()[n] = state.allocValue())
        ->mkApp(args[0], args[1]->listElems()[n]);
}
...
/* Filter a list using a predicate; that is, return a list containing
   ...
   returns true. */
static void prim_filter(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  state.forceList(
      *args[1], pos,
  ...
  for (unsigned int n = 0; n < args[1]->listSize(); ++n) {
    ...
    if (state.forceBool(res, pos,
                        "while evaluating the return value of the filtering "
                        "function passed to builtins.filter"))
...
static void prim_elem(EvalState &state, const PosIdx pos, Value **args,
                      Value &v) {
  bool res = false;
  state.forceList(
      *args[1], pos,
  ...
  for (auto elem : args[1]->listItems())
    if (state.eqValues(*args[0], *elem, pos,
                       "while searching for the presence of the given element "
                       "in the list")) {
      res = true;
      break;
    }
...
static void prim_concatLists(EvalState &state, const PosIdx pos, Value **args,
                             Value &v) {
  state.forceList(
      *args[0], pos,
      "while evaluating the first argument passed to builtins.concatLists");
  state.concatLists(
      v, args[0]->listSize(), args[0]->listElems(), pos,
...
static void prim_length(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  state.forceList(
      *args[0], pos,
...
static void prim_foldlStrict(EvalState &state, const PosIdx pos, Value **args,
                             Value &v) {
  state.forceFunction(
      *args[0], pos,
...
static void anyOrAll(bool any, EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
  state.forceFunction(
      *args[0], pos,
  ...
  for (auto elem : args[1]->listItems()) {
    ...
    bool res =
        state.forceBool(vTmp, pos,
                        std::string("while evaluating the return value of the "
                                    "function passed to builtins.") +
                            (any ? "any" : "all"));
    if (res == any) {
      v.mkBool(any);
      return;
...
static void prim_any(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
...
static void prim_all(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
...
static void prim_genList(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
...
static void prim_lessThan(EvalState &state, const PosIdx pos, Value **args,
                          Value &v);

static void prim_sort(EvalState &state, const PosIdx pos, Value **args,
                      Value &v) {
  state.forceList(
      *args[1], pos,
  ...
  auto comparator = [&](Value *a, Value *b) {
    ...
    if (args[0]->isPrimOp() && args[0]->primOp->fun == prim_lessThan)
      return CompareValues(
          state, noPos,
          "while evaluating the ordering function passed to builtins.sort")(a,
                                                                            b);
...
static void prim_partition(EvalState &state, const PosIdx pos, Value **args,
                           Value &v) {
  state.forceFunction(
      *args[0], pos,
  ...
  for (unsigned int n = 0; n < len; ++n) {
    ...
    if (state.forceBool(res, pos,
                        "while evaluating the return value of the partition "
                        "function passed to builtins.partition"))
...
static void prim_groupBy(EvalState &state, const PosIdx pos, Value **args,
                         Value &v) {
  state.forceFunction(
      *args[0], pos,
  ...
  for (auto vElem : args[1]->listItems()) {
    ...
    state.callFunction(*args[0], *vElem, res, pos);
    auto name =
        state.forceStringNoCtx(res, pos,
                               "while evaluating the return value of the "
                               "grouping function passed to builtins.groupBy");
    auto sym = state.symbols.create(name);
...
static void prim_concatMap(EvalState &state, const PosIdx pos, Value **args,
                           Value &v) {
  state.forceFunction(
      *args[0], pos,
  ...
  for (unsigned int n = 0; n < nrLists; ++n) {
    ...
    state.forceList(lists[n], lists[n].determinePos(args[0]->determinePos(pos)),
                    "while evaluating the return value of the function passed "
                    "to buitlins.concatMap");
...
static void prim_add(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
  state.forceValue(*args[0], pos);
  state.forceValue(*args[1], pos);
  if (args[0]->type() == nFloat || args[1]->type() == nFloat)
    v.mkFloat(state.forceFloat(
                  *args[0], pos,
    ...
    v.mkInt(
        state.forceInt(*args[0], pos,
                       "while evaluating the first argument of the addition") +
        state.forceInt(*args[1], pos,
                       "while evaluating the second argument of the addition"));
}
...
static void prim_sub(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
  state.forceValue(*args[0], pos);
  state.forceValue(*args[1], pos);
  if (args[0]->type() == nFloat || args[1]->type() == nFloat)
...
static void prim_mul(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
  state.forceValue(*args[0], pos);
  state.forceValue(*args[1], pos);
  if (args[0]->type() == nFloat || args[1]->type() == nFloat)
    v.mkFloat(
        state.forceFloat(*args[0], pos,
...
static void prim_div(EvalState &state, const PosIdx pos, Value **args,
                     Value &v) {
  state.forceValue(*args[0], pos);
  state.forceValue(*args[1], pos);

  NixFloat f2 = state.forceFloat(
      *args[1], pos, "while evaluating the second operand of the division");
  if (f2 == 0)
    state.debugThrowLastTrace(EvalError(
        {.msg = hintfmt("division by zero"), .errPos = state.positions[pos]}));
  ...
  if (args[0]->type() == nFloat || args[1]->type() == nFloat) {
    ...
    NixInt i1 = state.forceInt(
        *args[0], pos, "while evaluating the first operand of the division");
    NixInt i2 = state.forceInt(
        *args[1], pos, "while evaluating the second operand of the division");
    /* Avoid division overflow as it might raise SIGFPE. */
    if (i1 == std::numeric_limits<NixInt>::min() && i2 == -1)
      state.debugThrowLastTrace(
          EvalError({.msg = hintfmt("overflow in integer division"),
                     .errPos = state.positions[pos]}));
...
static void prim_bitAnd(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  v.mkInt(
      state.forceInt(
          *args[0], pos,
...
static void prim_bitOr(EvalState &state, const PosIdx pos, Value **args,
                       Value &v) {
  v.mkInt(state.forceInt(
              *args[0], pos,
...
static void prim_bitXor(EvalState &state, const PosIdx pos, Value **args,
                        Value &v) {
  v.mkInt(
      state.forceInt(
          *args[0], pos,
...
/* Convert the argument to a string.  Paths are *not* copied to the
   ...
   `"/nix/store/whatever..."'. */
static void prim_toString(EvalState &state, const PosIdx pos, Value **args,
                          Value &v) {
  NixStringContext context;
  auto s = state.coerceToString(
      pos, *args[0], context,
      "while evaluating the first argument passed to builtins.toString", true,
      false);
  v.mkString(*s, context);
}
...
/* `substring start len str' returns the substring of `str' starting
   ...
   non-negative. */
static void prim_substring(EvalState &state, const PosIdx pos, Value **args,
                           Value &v) {
  int start = state.forceInt(*args[0], pos,
                             "while evaluating the first argument (the start "
                             "offset) passed to builtins.substring");
  int len = state.forceInt(*args[1], pos,
                           "while evaluating the second argument (the "
                           "substring length) passed to builtins.substring");
  NixStringContext context;
  auto s = state.coerceToString(pos, *args[2], context,
                                "while evaluating the third argument (the "
                                "string) passed to builtins.substring");

  if (start < 0)
    state.debugThrowLastTrace(
        EvalError({.msg = hintfmt("negative start position in 'substring'"),
                   .errPos = state.positions[pos]}));

  v.mkString((unsigned int)start >= s->size() ? "" : s->substr(start, len),
             context);
}
...
static void prim_stringLength(EvalState &state, const PosIdx pos, Value **args,
                              Value &v) {
  NixStringContext context;
  auto s = state.coerceToString(
      pos, *args[0], context,
...
static void prim_hashString(EvalState &state, const PosIdx pos, Value **args,
                            Value &v) {
  auto type = state.forceStringNoCtx(
      *args[0], pos,
...
struct RegexCache {
  ...
  std::regex get(std::string_view re) {
    ...
    return cache
        .emplace(keys.back(), std::regex(keys.back(), std::regex::extended))
        .first->second;
  }
};
...
void prim_match(EvalState &state, const PosIdx pos, Value **args, Value &v) {
  ...
  try {
    ...
    if (e.code() == std::regex_constants::error_space) {
      ...
      state.debugThrowLastTrace(EvalError(
          {.msg =
               hintfmt("memory limit exceeded by regular expression '%s'", re),
           .errPos = state.positions[pos]}));
    } else
      state.debugThrowLastTrace(
          EvalError({.msg = hintfmt("invalid regular expression '%s'", re),
                     .errPos = state.positions[pos]}));
  }
}
...
void prim_split(EvalState &state, const PosIdx pos, Value **args, Value &v) {
  ...
  try {
    ...
    for (auto i = begin; i != end; ++i) {
      ...
      (v.listElems()[idx++] = state.allocValue())
          ->mkString(match.prefix().str());
      ...
      for (size_t si = 0; si < slen; ++si) {
        if (!match[si + 1].matched)
          ...
          (elem->listElems()[si] = state.allocValue())
...
static void prim_concatStringsSep(EvalState &state, const PosIdx pos,
                                  Value **args, Value &v) {
  NixStringContext context;

  auto sep =
      state.forceString(*args[0], context, pos,
                        "while evaluating the first argument (the separator "
                        "string) passed to builtins.concatStringsSep");
  state.forceList(*args[1], pos,
                  "while evaluating the second argument (the list of strings "
                  "to concat) passed to builtins.concatStringsSep");
  ...
  for (auto elem : args[1]->listItems()) {
    ...
    res += *state.coerceToString(
        pos, *elem, context,
        "while evaluating one element of the list of strings to concat passed "
...
static void prim_replaceStrings(EvalState &state, const PosIdx pos,
                                Value **args, Value &v) {
  state.forceList(
      *args[0], pos,
  ...
  if (args[0]->listSize() != args[1]->listSize())
    state
        .error("'from' and 'to' arguments passed to builtins.replaceStrings "
  ...
  for (auto elem : args[0]->listItems())
    from.emplace_back(
        state.forceString(*elem, pos,
                          "while evaluating one of the strings to replace "
                          "passed to builtins.replaceStrings"));
    ...
    for (; i != from.end(); ++i, ++j, ++j_index)
      if (s.compare(p, i->size(), *i) == 0) {
        ...
        if (v == cache.end()) {
          ...
          auto ts =
              state.forceString(**j, ctx, pos,
                                "while evaluating one of the replacement "
                                "strings passed to builtins.replaceStrings");
...
static void prim_parseDrvName(EvalState &state, const PosIdx pos, Value **args,
                              Value &v) {
  auto name = state.forceStringNoCtx(
      *args[0], pos,
...
static void prim_compareVersions(EvalState &state, const PosIdx pos,
                                 Value **args, Value &v) {
  ...
  auto version2 = state.forceStringNoCtx(*args[1], pos,
                                         "while evaluating the second argument "
                                         "passed to builtins.compareVersions");
...
static void prim_splitVersion(EvalState &state, const PosIdx pos, Value **args,
                              Value &v) {
...
#include "primops/derivation.nix.gen.hh"
      ...
      "\0";
  eval(parse(code, sizeof(code), derivationInternal, {CanonPath::root},
             staticBaseEnv),
       *vDerivation);
}
